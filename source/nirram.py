...
class NIRRAM:
    ...
    def __init__(
        ...
        hash = update_data_log(current_date, current_time, f"chip_{chip}_device_{device}", test_type, additional_info)
        self.datafile_path = f"{settings["path"]["data_header"]}/{test_type}/{current_date}_{chip}_{device}_{additional_info}_{hash}.csv"
        ...
        self.load_session(self,settings)
    ...
    def load_settings(self,settings):
        ...
        if "NISwitch" in settings:
            ...
            self.relays = niswitch.Session(settings["NISwitch"]["deviceID"])
    ...
    def load_session(self,settings):  
        ...
        # Initialize sessions and session_names as empty arrays
        sessions,session_names,session_numbers  = []
        ...
        for deviceID in settings["NIDigital"]["deviceID"]:
            session_id = int(deviceID[3:7])
            if isinstance(session_id, int):
                if session_id not in session_numbers:
                    session_numbers.append(session_id)
                    session_names.append(deviceID)
                else:
                    session_names[session_numbers.index(session_id)] += "," + deviceID
            
            for session_name in session_names:
                sessions.append(nidigital.Session(session_name))
            
            self.digital = sessions
        ...
        for digital in self.digital:
            if any("BL" in channel for channel in digital.Session.get_channel_names("0:32")):
                self.bl_dev.append(digital)
            if any("SL" in channel for channel in digital.Session.get_channel_names("0:32")):
                self.sl_dev.append(digital)
            if any("WL" in channel for channel in digital.Session.get_channel_names("0:32")):
                self.wl_dev.append(digital)
        ...
        for digital in self.digital:
            digital.channels[self.all_channels].write_static(nidigital.WriteStaticPinState.ZERO)
    ...
    def set_vbl(self, vbl_chan, vbl, vbl_lo):
        ...
        for digital in self.bl_dev:
            digital.channels[vbl_chan].configure_voltage_levels(vbl_lo, vbl, vbl_lo, vbl, 0)
        
        if self.debug_printout:
            print(f"Setting VBL: {vbl} on chan: {vbl_chan}")      
    ...
    def set_vsl(self, vsl_chan, vsl, vsl_lo):
        ...
        for digital in self.sl_dev:
            digital.channels[vsl_chan].configure_voltage_levels(vsl_lo, vsl, vsl_lo, vsl, 0)
        
        if self.debug_printout:
            print(f"Setting VSL: {vsl} on chan: {vsl_chan}")
    ...
    def set_vwl(self, vwl_chan, vwl_hi, vwl_lo):
        ...
        if self.debug_printout:
            print(f"Setting VWL: {vwl_hi} on chan: {vwl_chan}")
    ...
    def ppmu_set_vsl(self, vsl_chan, vsl):
        ...
        # assert(vsl_chan in self.all_sls)
        for digital in self.digital:
            digital.channels[vsl_chan].ppmu_voltage_level = vsl
        for digital in self.digital:
            digital.channels[vsl_chan].ppmu_source()
