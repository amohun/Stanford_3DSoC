...
class NIRRAM:
    ...
    def _initialize_logging(self, test_type: str, additional_info: str):
        ...
        hash_value = self.update_data_log(
            current_date,
            current_time,
    ...
    def _load_device_settings(self, settings_manager: SettingsUtil):
        ...
        self.wl_signals = settings_manager.get_setting("device.WL_SIGNALS", default=settings_manager.get_setting("device.WL_IN"))
        self.all_wl_signals = settings_manager.get_setting("device.all_WL_SIGNALS", default=settings_manager.get_setting("device.all_WL_IN"))
    ...
    def _load_relay_settings(self, settings_manager: SettingsUtil):
        ...
        self.all_channels_flat = [item for sublist in self.all_channels for item in sublist]
    ...
    def direct_read(
        ...
        self._define_measurement_dataframes(wls, res_array_bls)
        ...
        self.dbg.operation_debug("Direct Read", ["VBL, VWL, VSL, VB, VWL_UNSEL_OFFSET"], [vbl, vwl, vsl, vb, vwl_unsel_offset])
        ...
        for wl,wl_bls,wl_sls in zip(wls,bls,sls):
            ...
            self.digital_patterns.ppmu_set_voltage([[],[],["WL_UNSEL"]], [vsl + vwl_unsel_offset], sort=False) 
            ...
            for wl_i in all_wls:
                if wl_i in wl:
                    ...
                    self.ppmu_set_vwl(wl_i, vwl_unsel)
    ...
    def write_pulse(
        ...
        self.set_vwl(["WL_UNSEL"], vwl_hi=vwl_unsel, vwl_lo=v_base, debug = debug)
        ...
        self.digital_patterns.pulse(masks,pulse_lens=pulse_lens,max_pulse_len=max_pulse_len, pulse_groups=[["WL_IN"],["BL","SL","WL"],["WL_IN"]] )
    ...
    def select_memory_cells(self,cells,bls, bls_unselected=[], wls=None,debug=None):
        """
        Select Channels: This function is used to select the channels to be pulsed/Written to.
        The function takes in the cells to be pulsed, the BLs, and WLs to be used, and the
        ...
        # Use either given bls, wls, or values from TOML
        bls = bls or self.bls
        bls_unselected = bls_unselected # More specific work needed to adapt 1TNR
        wls = wls or self.wls
       
        # If WLS is a string (single WL), convert to a list of strings
        if isinstance(wls, str):
            wls = [wls]
        
        # If BLS is a list of strings (single BL), convert to list of lists of strings for each wordline
        ...
        sls = [[f"SL_{str(bl[3:])}" for bl in wl_bls] for wl_bls in bls]

        if cells is not None and len(cells) > 0 and all(len(cell)==2 for cell in cells): 
            # If cells are given, set the bls and wls based on the cells without repeating
            for cell in cells:
                wl, bl = cell
                if wl not in wls:
                    wls.append(wl)
                    bls.append([bl])
                    sls.append([f"SL_{str(bl[3:])}"])
                if bl not in bls[wls.index(wl)]:
                    bls[wls.index(wl)].append(bl)
                    sls[wls.index(wl)].append(f"SL_{str(bl[3:])}")
        cells = []
        # Define the cells as the list of tuples of WL and BL
        for wl, wl_bls in zip(wls, bls):
            for bl in wl_bls:
                cells.append((wl, bl))
        selected_memory_cells = RRAMCells(cells, wls, bls, bls_unselected, [bl for bl in bls if bl not in bls_unselected], sls)
    ...
    def setup_pulse(
        ...
        if masks is None:
            ...
            # For multiple worlines, creaste a list of masks which can be pulsed sequentially
            else:
                masks = []
                for wl, wl_bls, wl_sls in zip(wls,bls,sls):
                    masks.append(Masks(
        ...
        self.dbg.end_function_debug
        if masks:
            return pulse, masks.get_pulse_masks()
        else:
            return pulse, masks
    ...
    def update_pulse(
            ...
            self.dbg.start_function_debug(debug)
            # Check that the pulse is already defined, if not just set up new pulse
            if pulse is None:
                if mode is None: mode = "SET"
                pulse, _ = self.setup_pulse(masks, mode, bls, bls_unselected, wls, cells, wl_bl_sl_voltages, vbl_unsel, vwl_unsel_offset, pulse_width, init_setup, measure_res, target_res, print_data, average_resistance, record, debug)
                
                self.dbg.end_function_debug()
                return pulse
            
            # If the pulse is already defined, update the pulse with the new parameters #

            # Check then update channels
            if [bls, bls_unselected, wls, cells] != [pulse.bls, pulse.bls_unsel, pulse.sls, pulse.cells]:
                updated_channels = self.select_memory_cells(cells,bls, bls_unselected, wls)
                pulse.bls, pulse.bls_unsel, pulse.sls, pulse.wls = updated_channels

            # Check 
            if updated_voltages != [pulse.vwl, pulse.vbl, pulse.vbl_unsel, pulse.vsl, pulse.vwl_unsel, pulse.v_base]:
                updated_voltages = self.define_voltages_to_be_pulsed(mode, wl_bl_sl_voltages, vwl_unsel_offset, vbl_unsel)
                pulse.vwl, pulse.vbl, pulse.vbl_unsel, pulse.vsl, pulse.vwl_unsel, pulse.v_base = updated_voltages
                
            
            if target_res is not None:
                pulse.target = target_res
    ...
    def check_cell_resistance(self,res_array, wls, bls, sls, target_res, mode,print_info=False,debug=None):
    ...
    def dynamic_pulse(self, sessions=None, cells=None, bls=None, wls=None, mode="RESET", print_data=True,record=True,target_res=None, average_resistance = False, is_1tnr=False,bl_selected=None, voltages=[None,None,None],relayed=False,reset_after=False,debug=None):
        ...
        for wl,wl_bls,wl_sls in zip(wls,bls,sls):
            ...
            if self.relays:
                wl_signals,_ = self.relay_switch([wl], relayed=True, debug = debug)
            else:
                wl_signals = [wl]
...
def main():
    args = arg_parse()
    rram = NIRRAM(args.chip, args.device, polarity=args.polarity, settings=args.settings, test_type=args.test_type, additional_info=args.additional_info)
    print("NIRRAM Abstracted Class Loaded Successfully.")

    rram.dynamic_pulse(wls=["WL_0"],bls=["BL_0"],mode="SET",record=True,print_data=True,target_res=None,average_resistance=True,debug=False)
